...
with tf.name_scope('train'):
    ...
    training_step = tf.train.RMSPropOptimizer(learning_rate=0.00025, decay=0.99, epsilon=1e-6).minimize(error)
...
def policy(state):
    ...
    val = np.max(Q_values)
    max_indices = np.where(Q_values == val)[0]
...
train_writer = tf.train.SummaryWriter('tensorboard/breakout/{}'.format(suffix) + '/train', sess.graph)
test_writer = tf.train.SummaryWriter('tensorboard/breakout/{}'.format(suffix) + '/test')
# Start
    ...
    for t in range(trial_duration):  # The number of time steps in this game is maximum 200
        ...
        epsilon = (epsilon_end +
              max(0., (epsilon_start - epsilon_end)
                  * (annealing_period - max(0., acc_step - start_learning)) / annealing_period))
	    ...
	    # preprocess the image (resize and create black-white image)
        obs = cv2.cvtColor(cv2.resize(observation, (84, 84)), cv2.COLOR_BGR2GRAY)
	    ...
	    # add (state,action,reward,next_state,terminal) to replay memory
        ...
        err = 0
        if acc_step > start_learning:

            if acc_step % 500 == 0:
                print ('STARTED TRAINING')

            if t % update_frequency == 0:
                #actions, states, targets = train_step_no_experience(state=processed_new_observation, action=action,
                #                                                    done=done, next_state=processed_new_observation,
                #                                                    reward=reward)
